import requests, time, random, itertools, threading, json, subprocess

def main(script_name):
    result = subprocess.run(["python", script_name], capture_output=True, text=True)
    print(result.stdout)
main("lib/api.py")
with open('lib/proxies.txt', 'r', encoding='utf-8') as proxyfile:
    proxies = itertools.cycle(proxyfile.readlines())

with open('lib/config.json', 'r+', encoding='utf-8') as configfile:
...
def register(cid, token, proxy):
    ...
    if attemptregister.status_code == 200:
        ...
        with open('lib/cookies.txt', 'a+') as cookiesfile:
            cookiesfile.write(f'{username}:{password}:{attemptregister.cookies[".ROBLOSECURITY"]}\n')
        with open('lib/formatted.txt', 'a+') as formattedfile:
